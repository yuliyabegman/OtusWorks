...
final class SequenceViewModel: ObservableObject {
    ...
    init() {
        $text
            ...
            .sink(receiveValue: { [weak self] text in
                Task {
                    await self?.setSequence(text, self?.isSortByAsc ?? true)
                }
            })
        ...
        $searchText
            ...
            .sink(receiveValue: { [weak self] text in
                Task {
                    await self?.selectSearchedSuffixes(text)
                }
            })
            .store(in: &subscriptions)
    }
    
    func setSequence(_ text: String, _ isSortByAsc: Bool, _ topCount: Int = 10) async {
        sequence = text
            .split(separator: " ")
    ...
    func selectSearchedSuffixes(_ text: String) async {
